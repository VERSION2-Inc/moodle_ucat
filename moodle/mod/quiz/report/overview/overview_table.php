<?php
require_once $CFG->dirroot.'/blocks/cat_manager/lib.php'; // CATMOD
...
class quiz_report_overview_table extends table_sql {
    ...
    function col_fullname($attempt){
        ...
        return $html . '<br /><a class="reviewlink" href="review.php?q='.$this->quiz->id.'&amp;attempt='.$attempt->attempt.
            ...
            //CATMOD BEGIN
        //    . '<br/><a class="reviewlink" href="catrecord.php?attempt=' . $attempt->attempt . '">' . get_string('downloadcatrecord', 'block_cat_manager') . '</a>'
            //CATMOD END
        ;
    }
            ...
            if ($stateforqinattempt && question_state_is_graded($stateforqinattempt)) {
                ...
                if (!$this->is_downloading()) {
                    ...
                    if (($this->questions[$questionid]->maxgrade != 0)){
                        // CATMOD BEGIN
                        if (!cat_quiz_report_is_question_answered($stateforqinattempt)) {
                            return '--';
                        }
                        // CATMOD END
...
//             $this->gradedstatesbyattempt = quiz_get_newgraded_states($attemptids, true, 'qs.id, qs.grade, qs.event, qs.question, qs.attempt');
            $this->gradedstatesbyattempt = quiz_get_newgraded_states($attemptids, true, 'qs.id, qs.grade, qs.event, qs.question, qs.attempt, qs.answer'); // CATMOD
            if (has_capability('mod/quiz:regrade', $this->context)){
...
//             $this->gradedstatesbyattempt = quiz_get_newgraded_states($this->sql, true, 'qs.id, qs.grade, qs.event, qs.question, qs.attempt');
            $this->gradedstatesbyattempt = quiz_get_newgraded_states($this->sql, true, 'qs.id, qs.grade, qs.event, qs.question, qs.attempt, qs.answer'); // CATMOD
            if (has_capability('mod/quiz:regrade', $this->context)){
