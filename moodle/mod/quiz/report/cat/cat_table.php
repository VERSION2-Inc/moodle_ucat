...
class quiz_report_cat_table extends table_sql {
    ...
    function quiz_report_cat_table($quiz , $qmsubselect, $groupstudents,
                $students, $detailedmarks, $questions, $candelete, $reporturl, $displayoptions, $context){
        parent::table_sql('mod-quiz-report-cat-report');
        $this->quiz = $quiz;
    ...
    function wrap_html_start(){
        if (!$this->is_downloading()) {
            if ($this->candelete) {
                ...
                echo '<div id="tablecontainer" class="cat-tablecontainer">';
        ...
        if (!$this->is_downloading()) {
            ...
            if ($this->candelete) {
                ...
                if (has_capability('mod/quiz:regrade', $this->context)){
                    echo '<input type="submit" name="regrade" value="'.get_string('regradeselected', 'quiz_cat').'"/>';
                }
                echo '<input type="submit" onclick="return confirm(\''.$strreallydel.'\');" name="delete" value="'.get_string('deleteselected', 'quiz_cat').'"/>';
                echo '</div>';
                // Close form
                echo '</div>';
                echo '</form></div>';
    ...
    //CATMOD BEGIN
    public function col_layout($attempt) {
        global $DB, $CFG;

        $data = array();
        $qids = array_filter(explode(',', $attempt->layout));
        $i = 0;
        foreach ($qids as $qid) {
            $question = $DB->get_record('question', array('id' => $qid));
            if (cat_is_question_saved($attempt->attemptuniqueid, $i)) {
    ...
    public function col_ability($attempt) {
        return cat_logit2unit($attempt->ability, true);
    }

    public function col_se($attempt) {
        if ($attempt->se < CAT_DUMMY_SE) {
            return cat_diff_logit2unit($attempt->se, true);
        }
        return '-';
    }

    public function col_abilityrange($attempt) {
        if ($attempt->se < CAT_DUMMY_SE) {
            return cat_logit2unit($attempt->ability - $attempt->se)
                .'-'.cat_logit2unit($attempt->ability + $attempt->se);
        }
        return '--';
    }
    //CATMOD END
     ...
     * mod/quiz/report/cat/report.php to see what fields are present,
    ...
    function other_cols($colname, $attempt){
        ...
        if (preg_match('/^qsgrade([0-9]+)$/', $colname, $matches)){
            ...
            if ($stateforqinattempt && question_state_is_graded($stateforqinattempt)) {
                ...
                $text = get_string('requiresgrading', 'quiz_cat');
                if (!$this->is_downloading()) {
                    $link = new moodle_url("/mod/quiz/reviewquestion.php?attempt=$attempt->attempt&question=$question->id");
                    $action = new popup_action('click', $link, 'reviewquestion', array('height' => 450, 'width' => 650));
    ...
    function col_regraded($attempt){
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'quiz_cat');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'quiz_cat');
        }
    }
