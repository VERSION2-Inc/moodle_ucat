...
require_once($CFG->dirroot.'/mod/quiz/report/cat/catsettings_form.php');
require_once($CFG->dirroot.'/mod/quiz/report/cat/cat_table.php');

class quiz_cat_report extends quiz_default_report {
    ...
    function display($quiz, $cm, $course) {
        ...
        $pageoptions['mode'] = 'cat';

        $reporturl = new moodle_url('/mod/quiz/report.php', $pageoptions);
        $qmsubselect = quiz_report_qm_filter_select($quiz);

        $mform = new mod_quiz_report_cat_settings($reporturl, array('qmsubselect'=> $qmsubselect, 'quiz'=>$quiz,
        ...
        if ($fromform = $mform->get_data()) {
            ...
            set_user_preference('quiz_report_cat_detailedmarks', $fromform->detailedmarks);
            ...
            $detailedmarks = get_user_preferences('quiz_report_cat_detailedmarks', 1);
        ...
        $table = new quiz_report_cat_table($quiz , $qmsubselect, $groupstudents,
                $students, $detailedmarks, $questions, $candelete, $reporturl,
                $displayoptions, $this->context);
        $table->is_downloading($download, get_string('reportcat','quiz'),
                    "$COURSE->shortname ".format_string($quiz->name,true));
        if (!$table->is_downloading()) {
            // Only print headers if not asked to download data
            $this->print_header_and_tabs($cm, $course, $quiz, "cat");
        }
        ...
        if (!$nostudents || ($attemptsmode == QUIZ_REPORT_ATTEMPTS_ALL)) {
            ...
            $fields .='qa.uniqueid AS attemptuniqueid, qa.id AS attempt, ' .
                ...
                'qa.sumgrades, qa.timefinish, timestart, CAST(qa.timefinish AS SIGNED) - CAST(qa.timestart AS SIGNED) AS duration '
                //CATMOD BEGIN
                . ', cat.ability, cat.se, qa.layout';
                //CATMOD END
            ...
            //CATMOD BEGIN
            $from .= ' LEFT JOIN {cat_sessions} cat ON cat.attempt = qa.id';
            //CATMOD END
            ...
            if (!$table->is_downloading()) { //do not print notices when downloading
                ...
                if (has_capability('mod/quiz:regrade', $this->context)) {
                    ...
                    if ($currentgroup) {
                        ...
                        $regradealldrydolabel = get_string('regradealldrydogroup', 'quiz_cat', $a);
                        $regradealldrylabel = get_string('regradealldrygroup', 'quiz_cat', $a);
                        $regradealllabel = get_string('regradeallgroup', 'quiz_cat', $a);
                    } else {
                        $regradealldrydolabel = get_string('regradealldrydo', 'quiz_cat', $countregradeneeded);
                        $regradealldrylabel = get_string('regradealldry', 'quiz_cat');
                        $regradealllabel = get_string('regradeall', 'quiz_cat');
                    }
            ...
            //CATMOD BEGIN
            $columns[] = 'ability';
            $headers[] = get_string('ability', 'block_cat_manager');

            $columns[] = 'se';
            $headers[] = get_string('standarderror', 'block_cat_manager');

            $columns[] = 'abilityrange';
            $headers[] = get_string('probableabilityrange', 'block_cat_manager');

            $columns[] = 'layout';
            $headers[] = get_string('questionsadministered', 'block_cat_manager');
            //CATMOD END
            ...
            if (!$table->is_downloading() && has_capability('mod/quiz:regrade', $this->context) && $regradedattempts) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'quiz_cat');
            }
            if ($showgrades) {
                $columns[] = 'sumgrades';
        ...
        if (!$table->is_downloading() && $showgrades) {
            if ($currentgroup && $groupstudents) {
                ...
                if ($DB->record_exists_select('quiz_grades', "userid $usql AND quiz = ?", $params)) {
                     $imageurl = "{$CFG->wwwroot}/mod/quiz/report/cat/catgraph.php?id={$quiz->id}&amp;groupid=$currentgroup";
                     $graphname = get_string('catreportgraphgroup', 'quiz_cat', groups_get_group_name($currentgroup));
                     echo $OUTPUT->heading($graphname);
                     echo '<div class="graph"><img src="'.$imageurl.'" alt="'.$graphname.'" /></div>';
                }
            }
            if ($DB->record_exists('quiz_grades', array('quiz'=> $quiz->id))) {
                 $graphname = get_string('catreportgraph', 'quiz_cat');
                 $imageurl = $CFG->wwwroot.'/mod/quiz/report/cat/catgraph.php?id='.$quiz->id;
                 echo $OUTPUT->heading($graphname);
                 echo '<div class="graph"><img src="'.$imageurl.'" alt="'.$graphname.'" /></div>';
            }
        }
    ...
    function regrade_all($dry, $quiz, $groupstudents) {
        ...
        foreach ($questions as $question) {
            ...
            foreach ($attempts as $attempt) {
                ...
                $a->done = $attemptsdone;
                $a->todo = $attemptstodo;
                $apb->update($attemptsdone, $attemptstodo, get_string('attemptprogress', 'quiz_cat', $a));
            }
            $qsdone++;
            if (isset($qpb)) {
                $a = new stdClass();
                $a->done = $qsdone;
                $a->todo = $qstodo;
                $qpb->update($qsdone, $qstodo, get_string('qprogress', 'quiz_cat', $a));
            }
    ...
    function regrade_all_needed($quiz, $groupstudents) {
        ...
        if (!$attempts = $DB->get_records_sql('SELECT qa.*, qqr.questionid FROM {quiz_attempts} qa, {quiz_question_regrade} qqr WHERE '. $where, $params)) {
            echo $OUTPUT->heading(get_string('noattemptstoregrade', 'quiz_cat'));
            return true;
        }
        $this->clear_regrade_table($quiz, $groupstudents);
        // Fetch all questions
    ...
    function clear_regrade_table($quiz, $groupstudents) {
        ...
        if (!$DB->execute($delsql, $params)) {
            print_error('err_failedtodeleteregrades', 'quiz_cat');
        }
    }
    ...
    function check_overall_grades($quiz, $userids=array(), $attemptids=array()) {
        ...
        if (!$DB->execute($sql, $params)) {
            print_error('err_failedtorecalculateattemptgrades', 'quiz_cat');
        }
    ...
    function regrade_selected_attempts($quiz, $attemptids, $groupstudents) {
        ...
        if (!$attempts = $DB->get_records_sql('SELECT qa.* FROM {quiz_attempts} qa WHERE '. $where, $params)) {
            print_error('noattemptstoregrade', 'quiz_cat');
        }
